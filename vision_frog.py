...
FROG_OFFSET_X = -10  # سالب = يسار
FROG_OFFSET_Y = -12   # سالب = فوق

def get_frog_balls_by_color(frame):

    adjusted_center = (
    BACK_BALL_CENTER[0] + FROG_OFFSET_X,
    BACK_BALL_CENTER[1] + FROG_OFFSET_Y
)
    ...
    for color_name, bgr_color in COLOR_DRAW_MAP.items():
        ...
        for cnt in contours:
            ...
            if area > BALL_MIN_AREA:
                ...
                circularity = 4 * np.pi * \
                    (area / (perimeter * perimeter)) if perimeter > 0 else 0
...
def get_color_mask(hsv_roi, color_name):
    ...
    if color_name == "Green":
        ...
        return cv2.inRange(hsv_roi, np.array([10, 150, 150]), np.array([22, 255, 255]))
